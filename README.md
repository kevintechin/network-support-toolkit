...
| Backlog | 20 items open — [docs/backlog.md](docs/backlog.md): nine changes to the tool (#32 the DHCP server, #34 the launcher over a console-mode fallback, #35 the summary's *All checks passed*, #36 a stray value binding to `-ConfigPath`, #38 the first run losing the retransmission measurement to a cold counter provider, #40 the report explaining a badge it does not carry, #43 nothing telling the person that the reports are being written into a synced folder, #44 the launcher's timestamp in the machine's own date format, #45 the IT panel checking a typed target only after the run has started), seven in `tests/` and the build (#21 a reproducible asset, #25 the campaign's `M8` message and bundle name, #29 the campaign's manual steps, #30 the Mark of the Web on a CI artifact, #33 tiers 2 and 3 of the document-fact step, #41 the file table's reverse direction, #42 nothing proving the tool changes nothing), one machine nobody has yet (#31 AppLocker actually enforcing) and three decisions (#22 signing, #37 what the package carries for the person whose launcher was refused before it ran, #39 whether a measurement that could not be taken should outrank one that failed). Numbers 1 to 20, 23, 24 and 26 to 28 are closed |
