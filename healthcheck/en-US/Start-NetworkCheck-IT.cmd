...
if not exist "%SCRIPT%" (
    ...
    set "SUGGESTED=extract the complete ZIP file to a local folder, then run Start-NetworkCheck-IT.cmd again."
    goto :launcher_error
)
...
goto :ran
:run_captured
"%PS_EXE%" -NoLogo -NoProfile -ExecutionPolicy Bypass -STA -File "%SCRIPT%" -Interactive -ExpandDetails 2>>"%LOGFILE%"
:ran
set "RC=%ERRORLEVEL%"
