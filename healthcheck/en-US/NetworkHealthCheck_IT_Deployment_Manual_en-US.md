...
| **Configuration Thresholds** | Warning | Present only when a count, a timeout, a threshold or a check flag had the wrong type or was out of range, or a warning level sits above its critical level; the details name each one and say what was used instead |
...
| `-PingTarget`, `-DnsName`, `-TcpTarget` (host:port) | Extra targets; several values separated by commas, semicolons or spaces, e.g. `-PingTarget 10.0.0.1,10.0.0.2` |
| `-HttpUrl` | Extra URLs; several separated by spaces only, because commas and semicolons are legal inside a URL |
