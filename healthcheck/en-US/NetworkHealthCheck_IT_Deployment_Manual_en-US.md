# Network Health Check 1.2.15 — IT Deployment Manual
...
```text
certutil -hashfile NetworkHealthCheck-1.2.15.zip SHA256
```

or in PowerShell `Get-FileHash NetworkHealthCheck-1.2.15.zip`. The asset is built from the repository's tracked files only, so it contains no report and no other output of a run.
...
| **AppLocker script rules** | Measured on a Windows 10 Pro machine on 2026-09-15, the first on which enforcement has taken effect here: an enforced rule that does not allow the folder the package sits in refuses **the launcher itself**, not the script it starts. AppLocker's script rules cover `.cmd` as well as `.ps1`, so `Start-NetworkCheck.cmd` never runs: cmd prints *This program is blocked by group policy. For more information, contact your system administrator.*, exits 1, and **no file of ours is written at all** — no error report, no PowerShell messages file, no environment report, no report. A rule that allows the folder but denies the `.ps1` leaves the launcher running and is the *AllSigned* row's case above. One more thing an enforced DLL or EXE rule can do short of stopping the script: refuse the C# compiler or the in-memory assembly the wireless retry reader is built with — that one row is then *Unable to Check* and nothing else changes | **Nothing, where the launcher is what was refused** — the line Windows printed is all there is, and the package carries nothing further for that case by decision (backlog #37): a page inside it would explain what that line already says and produce none of the evidence that is missing. **Ask for a photo of the window**, or have the person open a `cmd` window and run `Start-NetworkCheck-Console.cmd` from it so the text stays on screen. Where the script was refused and the launcher ran, the two files above |
...
- **Backlog** — <https://github.com/kevintechin/network-support-toolkit/blob/v1.2.15/docs/backlog.md>: what is known and not yet done, and what would close each item. It is in the repository rather than in the package, like the application-control page of section 8, because it changes between releases.
- **Thresholds** — <https://github.com/kevintechin/network-support-toolkit/blob/v1.2.15/docs/thresholds.md>: where the shipped threshold values come from — nowhere external, by a recorded decision — and what the published standards do and do not supply, each verified against its source on a stated date. It is in the repository for the same reason.
...
*NetworkHealthCheck 1.2.15. This manual describes the tool as shipped and the behaviour measured for this release; the rules quoted here are the code's, and the technical guide states them in full.*
