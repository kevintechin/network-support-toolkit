# Network Health Check 1.2.12 — IT Deployment Manual
...
| `NearEndTarget` | `Name`, `Address`, `Required` | `false` | One object, not a list: the near-end rung of the ping ladder - an IPv4 address on the users' own subnet that is not the gateway, pinged first. Empty as shipped; see *The near-end rung* below. A name, a placeholder or an address outside the machine's subnets is not probed, and the row says why |
...
or in PowerShell `Get-FileHash NetworkHealthCheck-1.2.12.zip`. The asset is built from the repository's tracked files only, so it contains no report and no other output of a run.
...
| **AppLocker script rules** | Not measured to take effect: the one machine that reported an enforcing policy denying the script ran it unrestricted (backlog #31 on the repository's backlog page, section 10). Whether an enforced rule refuses the script or runs it in the restricted mode above has not been observed | Whichever of the two files above the outcome produces |
...
- **Backlog** — <https://github.com/kevintechin/network-support-toolkit/blob/v1.2.12/docs/backlog.md>: what is known and not yet done, and what would close each item. It is in the repository rather than in the package, like the application-control page of section 8, because it changes between releases.
- **Thresholds** — <https://github.com/kevintechin/network-support-toolkit/blob/v1.2.12/docs/thresholds.md>: where the shipped threshold values come from — nowhere external, by a recorded decision — and what the published standards do and do not supply, each verified against its source on a stated date. It is in the repository for the same reason.
...
*NetworkHealthCheck 1.2.12. This manual describes the tool as shipped and the behaviour measured for this release; the rules quoted here are the code's, and the technical guide states them in full.*
