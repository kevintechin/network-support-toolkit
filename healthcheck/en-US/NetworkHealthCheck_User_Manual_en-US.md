...
**What the report contains.** The computer name, your user name, the Windows and PowerShell versions, the organization name IT put in the configuration and the company standards it set there (allowed addresses and subnets, gateways, DNS servers, the expected DHCP mode); every connected network adapter with its model, MAC and IP addresses, network profile name, gateways, DNS servers, DHCP mode and driver version; the Wi-Fi network name (SSID), the access point's hardware address (BSSID), band, channel and signal; the router's hardware address; the proxy settings Windows is using; the test targets by name and address, the addresses they resolved to or landed on, and the routers on the traceroute's first hops; the folder paths of the configuration file and the reports, which can include your user profile folder; and the text of any error the tool met. Handle the file the way your company's rules say to handle that kind of information. The tool never uploads it anywhere; it only writes it to the report folder, which is on this computer unless IT chose a folder elsewhere.
