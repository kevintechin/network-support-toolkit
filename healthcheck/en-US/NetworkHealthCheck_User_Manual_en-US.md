...
**What the report contains.** The computer name, your user name, the network adapters with their MAC and IP addresses, gateways and DNS servers, the names of your network profiles and the adapters' driver versions, the Wi-Fi network name (SSID) and the access point's hardware address (BSSID), the proxy settings Windows is using, the addresses the tests were aimed at and of the routers on the traceroute's first hops, the folder paths of the configuration file and the reports (which can include your user profile folder), and the text of any error the tool met. Handle the file the way your company's rules say to handle that kind of information. The tool never uploads it anywhere; it only writes it to the report folder, which is on this computer unless IT chose a folder elsewhere.
