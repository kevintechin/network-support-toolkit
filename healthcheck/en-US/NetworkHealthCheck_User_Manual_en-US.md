...
**What the report contains.** *Identity and the run:* the computer name, your user name, the Windows and PowerShell versions, the organization name IT put in the configuration, the time of the run, which entry was used and any targets typed into the IT panel, and the folder paths of the configuration file and the reports, which can include your user profile folder. *Every connected network adapter:* model, MAC and IP addresses, network profile name, gateways, DNS servers, DHCP mode, link speed, driver version and traffic counters. *Wi-Fi:* the network name (SSID), the access point's hardware address (BSSID), band, channel, rates and signal. *The network around you:* the router's hardware address, the routes in use, the proxy settings Windows is using, and the company standards IT set (allowed addresses and subnets, gateways, DNS servers, the expected DHCP mode). *The tests:* the targets by name and address, the addresses they resolved to or landed on, the routers on the traceroute's first hops, and the timing of the probes that answered. *Errors:* the text of any error the tool met and, in the JSON file only, its technical details, which name the program's own folder. Handle the file the way your company's rules say to handle that kind of information. The tool never uploads it anywhere; it only writes it to the report folder, which is on this computer unless IT chose a folder elsewhere.
...
| The black window names an **error report** (`LauncherError.txt`) | The launcher wrote what happened — the time, the computer, the user and the reason, plus the folder and the program's path when the file could be written beside the launcher — with a **Suggested action** line that fits the reason | Follow the suggested action, and send the file with your request. It is next to the launcher, or — when that folder cannot be written — in the Windows temporary folder as `NetworkHealthCheck_LauncherError.txt`; the window prints the path |
