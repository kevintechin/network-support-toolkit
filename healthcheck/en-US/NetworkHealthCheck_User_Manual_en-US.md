...
**A row is called Near-end host. What is it?** A computer or printer on your own network that IT chose and put in the configuration file, pinged before the router is. The ping rows read like a ladder - that host, then your router, then the public address - and each rung includes the path of the one before it: when the near-end host answers and the router does not, the trouble is at the router rather than in your cable or Wi-Fi; when neither answers, it is the link. The row's details say which parts of the path its pings crossed. If IT did not configure one, the row is not there; if the configured host is not on the network you are on right now - a laptop at home with the office settings - the row says so and counts for nothing.
