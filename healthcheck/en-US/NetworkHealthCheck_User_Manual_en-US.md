...
**It changes nothing.** The tool reads network information and runs connection tests — a few pings, a name lookup, a few TCP connections and one HTTPS request. It does not modify your IP address, DNS, routes, firewall, proxy or adapter settings, and it installs nothing. Administrator rights are normally not required.
...
Under the verdict, the report has a section called **What to tell IT**: a title and two to six lines. **Read this first** — it is the tool's own reading of where the fault is, written for you, and its last line always reminds you which file to send and what it contains.
...
| **Gateway does not answer** | The default gateway is configured but did not answer the pings sent to it, or lost too many of them. The second line depends on whether IT configured a near-end host (section 9) and it was reached through the same connection as the router: if that host answered normally, *the local path carried traffic during this run; what did not answer is the gateway itself*; if it lost its replies too, *that points at the local path before the gateway: link, Wi-Fi, switch or access point - or at the host itself*; without one, *the fault is between this computer and the router: link, Wi-Fi, switch, or the router itself*. Check the link light or Wi-Fi signal and whether other devices reach the router. A gateway that fails this check - not answering pings sent to itself - is a suspect, not a conviction: it may be forwarding traffic normally while dropping or rate-limiting those pings. A connection that succeeded proves that only if its route ran through this gateway - a same-subnet host, a VPN or a proxy can succeed without touching it - so check the link to it first and treat the gateway as unproven rather than broken. A gateway that answered every ping but slowly is not this title: it is *Connected, but quality is poor* (in earlier versions it was titled as one that did not answer). |
...
| **Connected, but quality is poor** | Connectivity works, but packet loss, latency, retransmissions, or adapter errors were above the thresholds. Typical causes: weak Wi-Fi, a congested link, or a faulty cable or port. Run the tool again while the problem is occurring and compare the numbers. A gateway that answered every ping, but slowly, lands here too: it was reached, so its slowness is a quality finding rather than a failure to answer. |
...
**What the report contains.** *Identity and the run:* the computer name, your user name, the Windows and PowerShell versions and PowerShell's language mode, the organization name IT put in the configuration, the time of the run, which entry was used, the run options (ping count, ping ceiling, sample length, how often the counters are read again inside it, traceroute hops, which optional checks ran) and any targets typed into the IT panel, and the folder paths of the configuration file and the reports, which can include your user profile folder. *Every connected network adapter:* name and model, MAC and IP addresses, network profile name, gateways, DNS servers, DHCP mode and the server that answered the DHCP lease, link speed, driver version, date and maker, traffic and error counters, and the technical fields beside them (interface index, adapter type, media type, data source). *Wi-Fi:* the network name (SSID), the access point's hardware address (BSSID), radio type, band, channel, rates and signal. On a wireless computer, also the adapter's 802.11 retry counts over the run — how many frames it had to send again, of how many it sent — and the retry rate they make (section 9 says what that number is and is not), and the access point's address as sampled at three moments of the run, so that the report can say whether the computer changed access point during the test (section 9). *The network around you:* the router's hardware address, the routes in use, the proxy settings Windows is using, the company standards IT set (allowed addresses and subnets, gateways, DNS servers, the expected DHCP mode) and any configuration warning, which quotes the value it objects to. *The tests:* the targets by name and address, the addresses they resolved to or landed on, the source address and adapter the route table selected for each ping target — which is what Windows would choose for it, not proof of the path the replies took —, the routers on the traceroute's first hops, the result of every test (replies, loss, latency and how unevenly the replies arrived, connection times, HTTP status) with the timing of the probes that answered, the computer-wide TCP send and retransmission counters, and, where a row carries them, the method used and the command that repeats it by hand. *Errors:* the text of any error the tool met and, in the JSON file only, its technical details, which name the program's own folder. Handle the file the way your company's rules say to handle that kind of information. The tool never uploads it anywhere; it only writes it to the report folder, which is on this computer unless IT chose a folder elsewhere — and when that folder is synced, Windows copies it to the cloud from there, as above.
...
| The black window says the program **ended with exit code** *something else* and that PowerShell or company security policy may have blocked it | Windows PowerShell refused to run the program. What it said is printed under the error and kept in a file beside the launcher, `PowerShellMessages_<time>.txt`, so you do not have to copy it off the screen | Send `LauncherError_<time>.txt` and `PowerShellMessages_<time>.txt` — both next to the launcher, both named in the window — to IT with your request. If the message says the file is *not digitally signed* or *blocked by a policy*, IT has to allow the program |
| The black window names an **error report** (`LauncherError_<time>.txt`) | The launcher wrote what happened — the time, with this computer's date format beside it, the computer, the user, the folder, the program's path, the PowerShell it used, whether PowerShell said anything and the reason — with a **Suggested action** line that fits the reason. Every attempt writes its own file: the number in the name is the date and time as this computer writes them, so nothing is overwritten if you try again | Follow the suggested action, and send the file — with the `PowerShellMessages_<time>.txt` beside it when the window names one — with your request. They are next to the launcher, or — when that folder cannot be written — in the Windows temporary folder as `NetworkHealthCheck_LauncherError_<time>.txt` and `NetworkHealthCheck_PowerShellMessages_<time>.txt`; the window prints the paths. The file's own *Date modified*, as your file manager shows it, is the reliable time of the attempt |
| No window appears; the check runs as text in the black window instead, and at the end the window says it stays open and waits for a key | The graphical interface could not start on this computer, and the tool switched to text mode by itself. The same checks ran, the reports were written to the usual folder, and the report has a warning row saying so | Read the report paths on the screen, press a key to close the window, then open the `Reports` folder (section 5) and send the newest report as usual. `Start-NetworkCheck-Console.cmd` in the `en-US` folder runs the same checks in text mode from the start |
...
**Which addresses does it contact?** With the settings as shipped: your own default gateway, the public address `1.1.1.1` (a ping, a connection to port 443, and the first three hops toward it for the traceroute) and `https://www.microsoft.com/` (one page request through the proxy Windows is set to use, following any redirect it is given), plus the name lookup for `www.microsoft.com` through this computer's DNS servers. If IT configured a near-end host, that too: a computer or printer on your own network, pinged by its address. IT may have replaced these with the company's own services — the report's rows name the targets that were configured and, for a page request, the address it finally landed on.

**A row is called Near-end host. What is it?** A computer or printer on your own network that IT chose and put in the configuration file, pinged before the router is. The ping rows read like a ladder - that host, then your router, then the public address - and each rung includes the path of the one before it: when the near-end host answers and the router does not, the trouble is beyond your cable or Wi-Fi - at the router, or in the way it answers pings; when neither answers, the link is the first suspect. The row's details say which parts of the path its pings crossed. If the tool cannot tell that the pings went straight to that host - a VPN may have carried them - the row says so and does not count as the near-end host. If IT did not configure one, the row is not there; if the configured host is not on the network you are on right now - a laptop at home with the office settings - the row says so and counts for nothing.
...
**The TCP retransmission row says more seconds than IT configured.** The row names the seconds it really measured and, beside them, the length that was configured — the two are not the same number, because the sample runs from one counter reading to the next and the configured length is a minimum rather than a target. When a counter read had to be tried again, the row also says how many of those seconds went on the attempt that failed and which read it was. The traffic figures beside it are the counts over the window the row names, whatever its length. Where the sample counted retransmissions, the row also says where inside it they fell: the counters are read again every couple of seconds while the sample runs, and the row lists those stretches and names the one that held the most retransmissions, with its share of the count and its share of the time. Those lines place the retransmissions in time — a large share of the count in a small share of the time means they came in a burst — and they change nothing else on the row.
...
**What is the wireless retry percentage?** On a Wi-Fi computer the report also counts, over the run, the frames the wireless adapter had to send again because no acknowledgement came back, over every frame it tried to send: the air between this computer and the access point, for everything the computer was doing at the time. It is neither the TCP retransmission percentage, which counts TCP segments on any kind of link, nor the ping loss figure — a frame sent again usually arrives, only later — and it changes no verdict, because no one has established what number is bad. Read it beside the signal, and beside the same number from another day at the same desk.

**The report says the Wi-Fi access point changed during the test, or that a change cannot be seen.** In the IT diagnostics the tool records which access point your computer was connected to at three moments of the run — before the first test, in the middle and after the last — and compares them. The same access point at all three is the ordinary case. A different one under the same network name means your computer switched access points during the run, which can explain poor numbers with a good signal; a different network name means it moved to another network. The sentence about a change that cannot be seen is a limit stated on every report, not a finding: a switch out and back between two of those moments leaves nothing to see. None of it changes the verdict.

**A ping row has a line called Spread.** Beside the average, minimum and maximum, each ping row with at least two replies says how unevenly they arrived — the standard deviation of their times — and over how many replies. Two things to know when reading it: the times are whole milliseconds, so a spread of about 1 ms on a link that answers in 1 or 2 ms is the clock, not the network; and four replies describe four moments rather than the link, so the line says how many replies it takes (nine) before the figure describes the link. It has no threshold and changes no verdict.

**The TCP connection row lists several times and a retransmission count.** A connection test that succeeds is repeated — four connections with the settings as shipped, the number IT set as the ping count — and the row lists how long each one took and, for each, how many times Windows had to send its opening request again before it was answered, as the connection itself counted it. That count belongs to this computer and that one target at that moment — the one retransmission figure in the report that belongs to a single destination, where the *TCP retransmissions* rows count everything the computer sent. On a Windows 10 older than version 1703 the connection cannot be asked; the row then says so and instead reads each time against the delay after which Windows would have sent the request again (1000 ms unless IT changed it; the row says whether it read that value or assumed it), calling the result what it is, a stand-in and not a count. It changes no verdict: the row's result still follows whether the first connection worked, as before, and if one of the repeated connections failed the row says which one and stops there.
