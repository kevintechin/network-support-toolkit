...
**The TCP connection row lists several times and a retransmission timeout.** A connection test that succeeds is repeated — four connections with the settings as shipped, the number IT set as the ping count — and the row lists how long each one took and how many of them took at least as long as Windows waits before sending a connection request a second time (1000 ms unless IT changed it; the row says whether it read the value or assumed it). A connection that took that long is read as one whose first request went unanswered, on the path between this computer and that one target, at that moment — the one retransmission figure in the report that belongs to a single destination, where the *TCP retransmissions* rows count everything the computer sent. It changes no verdict: the row's result still follows whether the first connection worked, as before, and if one of the repeated connections failed the row says which one and stops there.
