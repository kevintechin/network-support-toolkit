...
**必要的閘道目標是由閘道自己的 stack 回應的（1.2.11、backlog #58）。** `AUTO_GATEWAY` 是唯一以 `Required` 出貨的目標，而送到設備自己位址的 echo 是由該設備的控制平面回應的，網路設備通常會對這種流量限速或降低優先權。所以閘道爽快回應是「近端路徑正常」的好證據；閘道不回應卻不是「它壞了」的證明。這項檢查維持必要——連自己閘道都到不了的機器通常真的有問題值得回報，而另一個選項（不計權重的列）會讓純 Wi-Fi 機器唯一的近端量測什麼都推不動（2026-09-10 決定）——而 1.2.11 起，失敗那一列的詳細資料與「要告訴 IT 的話」會用支援工程師現場手冊的說法說明這個失敗能說明什麼、不能說明什麼：嫌疑，不是定罪。
