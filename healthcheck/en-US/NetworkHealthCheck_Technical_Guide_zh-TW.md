# NetworkHealthCheck Portable 1.2.10：功能、設計、驗證與限制

## 1. 文件目的

本文件說明 `NetworkHealthCheck` 免安裝工具的功能、架構、判定方式、錯誤處理、驗證方法、已知限制與原始碼註解策略。文件對應版本 **1.2.10**，適用於套件中的繁體中文版與英文版；兩者的執行邏輯相同，只有使用者可見文字、預設測試名稱與註解語言不同。
...
**探測是從哪一張網路卡送出的（1.2.9、backlog #59）。** 每一列 ping 的詳細資料會寫出路由表為該目標選出的來源位址與介面，以 `Find-NetRoute -RemoteIPAddress <目標>` 在該目標的探測前後各讀一次。這是「**選擇**」而不是「**觀察**」：查詢回答的是查詢當下系統會選哪一條，而探測隨後是未綁定來源送出的，所以在多網卡機器上 —— 接上 dock、Wi-Fi 漫遊、VPN 剛連上 —— 路由可能在兩次查詢之間改變。因此這一列從不宣稱回應實際走的就是查詢回傳的那一條。兩次查詢不一致時 —— 包含一次查到、另一次沒查到 —— 這一列會報告這個改變，並說明它無法判斷實際是哪一個承載了探測。若 `Find-NetRoute` 不存在、沒有回傳路由或查詢失敗，這個資料會標示為無法取得並附上原因，ping 的量測不受影響 —— 這是本工具每一個 NetTCPIP 呼叫都適用的處理方式。要讓它成為「觀察」就必須綁定來源，那需要 `ping.exe -S`，本工具刻意不這麼做。
