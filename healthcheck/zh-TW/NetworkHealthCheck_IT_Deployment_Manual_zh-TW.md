...
**主機名稱可以含什麼（待辦 #54）。** 目標的主機——`PingTargets`、`DnsNames`、`TcpTargets`，以及 `HttpTargets` 網址裡的主機——只在「送到線上的名字就是設定的名字」時才會被檢測：標籤經 IDNA 編碼後只有字母、數字、連字號和底線，任何 IDNA 能編碼的文字都可以，以「.」或句號「。」分隔；IPv6 字面值可帶數字的 zone id（`fe80::1%12`），冒號和百分號只准出現在那裡。IDNA 會移除（零寬空白或連接子、軟連字號——從文件貼上來的值可能帶著）、改寫（全形字母、`ß`）或拒絕（雙向控制字元）的字元會讓這個值無法使用，該列會寫出是哪個字元（碼位與位置），好讓你修正設定；工具不會改寫名稱。格式正確但解析不到的名稱是相反的情形：它被問了，回答就是量測。完整規則在技術指南。
