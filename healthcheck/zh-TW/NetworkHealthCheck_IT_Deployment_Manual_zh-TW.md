...
| `NearEndTarget` | `Name`、`Address`、`Required` | `false` | 一個物件，不是清單：ping 階梯的近端那一階——使用者自己子網段上、不是閘道的一個 IPv4 位址，第一個被 Ping。出廠是空的；見下面的「近端那一階」。名稱、佔位符、閘道、這台電腦自己的位址、子網段的網路位址或廣播位址，或不在這台機器任何子網段上的位址，都不會被探測，那一列會說為什麼 |
