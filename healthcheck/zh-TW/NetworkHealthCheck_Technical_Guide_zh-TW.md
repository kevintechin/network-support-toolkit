...
取樣時間由**該通訊協定自己的兩個時間戳**計算，而不是由包住它們的快照時間戳計算。兩個通訊協定是循序讀取的，每個時間戳都在自己的讀取回來時取得，因此耗盡時限的讀取會落在它自己這個通訊協定、以及排在它後面那個通訊協定的窗內。增量不受影響——每個增量都是該通訊協定自己兩次讀數的差——比例又是比值而不是時間，所以失敗的讀取一直以來只弄壞了時間長度這一個數字。該列會把量到的時間長度與設定的 `RetransmissionSampleSeconds` 並列，後者是最短時間，不是目標值。
