...
- **基準值**快照在取樣窗開始前，每個類別另外做一次會被丟棄的讀取；它的讀數不採用，失敗既不是錯誤、也不算量測嘗試。效能計數器提供者第一次查詢要付的成本因此落在窗外，在那裡它拉長不了窗所回報的數字。結束快照不做這件事：在窗內做，等於拉長它本來要保護的東西。
...
取樣時間由**該通訊協定自己的兩個時間戳**計算，而不是由包住它們的快照時間戳計算。兩個通訊協定是循序讀取的，每個時間戳都在自己的讀取回來時取得，因此會拉長某個窗的，是那些延後了它的結束時間戳、卻沒有延後它的起始時間戳的讀取：在**結束**快照裡，是排在該通訊協定之前（含自己）的每次讀取；在**基準**快照裡，則是排在它*之後*的每次讀取——那些延遲會把後面的起始時間戳連同整段執行一起往後推。因此耗盡時限的 TCPv6 基準讀取會落在 TCPv4 的窗內，而不是它自己的窗內；那些較後面通訊協定的窗前讀取也依同樣方式計算。增量不受影響——每個增量都是該通訊協定自己兩次讀數的差——比例又是比值而不是時間，所以失敗的讀取一直以來只弄壞了時間長度這一個數字。該列會把量到的時間長度與設定的 `RetransmissionSampleSeconds` 並列，後者是最短時間，不是目標值。
