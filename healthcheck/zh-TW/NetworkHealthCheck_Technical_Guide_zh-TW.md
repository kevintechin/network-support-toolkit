...
語意驗證包含：IPv4、CIDR、前綴長度、閘道、DNS、DHCP 布林值、TCP 主機/連接埠、HTTP/HTTPS URL、DNS 目標及主要逾時值。部分不合理門檻會列為警告。語意錯誤目前不會把相關設定自動全部刪除，因此錯誤設定仍可能使部分結果失去判斷意義；報告會明確提示。
...
收集欄位包括介面名稱、描述、介面索引、連線速度、MAC、網路設定檔、IPv4/IPv6、前綴、閘道、DNS、DHCP 與資料來源。具有 IPv4 且有閘道的介面優先視為主要介面；若沒有，退而使用具有 IPv4 的介面。
...
- `Get-ExceptionDetails` 盡可能記錄例外型別、訊息、最多五層內部例外、位置與腳本堆疊。
...
程式主要由 56 個命名函式組成，依功能分成：輔助函式、設定、系統資料、規範比對、主動測試、計數器、報告、執行協調與 GUI。
...
11. **報告含敏感資訊。** 可能包含電腦名稱、使用者、MAC、IP、DNS、內部服務名稱與例外堆疊；外傳前需依公司政策處理。
