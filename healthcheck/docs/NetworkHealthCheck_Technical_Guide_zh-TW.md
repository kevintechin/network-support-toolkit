...
**當一個判定會靠單一個封包撐著時，就把它收回來。** 這一列保留量到的每一個數字、並停止決定整體結果，條件是下面兩件事**同時**成立：取樣數小於警告門檻所需的次數，也就是在那裡一個封包就值一整級；**而且**少遺失一次的話分類就會不一樣，也就是這個判定「就是」那一個封包。只成立一件，判定仍然成立 —— 4 次遺失 3 次是 75%，少一次還有 50%，沒有任何東西是靠那一個封包撐著的；21 次遺失 2 次確實會因一個封包而改變級別，但 21 次是 5% 門檻撐得住的取樣數，9.5% 是一次量測。完全沒有回覆的目標永遠走不到這條規則，因為 100% 遺失在任何次數下都是結論。而被收回的遺失判定不會蓋掉延遲的判定：這一列會交給延遲規則，若達到其中一個門檻就保有權重。
