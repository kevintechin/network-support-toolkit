...
Assert-Equal '#62 assoc: neither reader listing anything is the one no-interface row, its first line the none token and, since round 4, the reader''s reason' ("{0}/{1}" -f $noneAtAll.Count, ((($noneAtAll[0].Details -split "`r`n|`n")[0]) -match '[:：]\s*none;\s*wlanapi=open\s*$')) '1/True'
