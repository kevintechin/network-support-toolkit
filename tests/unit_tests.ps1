...
function New-ApiReading62($state, $query, $channel = 149, $software = 'on', $hardware = 'on', $error = '', $guid = 'e6b08c8a-3feb-4c3e-88c3-dee94dd2f0eb') { return [pscustomobject]@{ Timestamp = (Get-Date); Interfaces = @($(if ($error) { @() } else { [pscustomobject]@{ Guid = $guid; Description = 'Fixture AX211'; State = $state; Channel = $channel; RadioSoftware = $software; RadioHardware = $hardware; ConnectionQuery = $query } })); Error = $error; ErrorText = $(if ($error) { 'fixture' } else { '' }); Diagnostics = '' } }
...
Assert-Equal '#62 view: netsh failing while the service answers the connection query is not the location case' ("{0}/{1}/{2}" -f $view62b[0].Connected, $view62b[0].NetshFailed, $view62b[0].Refused) 'True/True/False'
...
$noneAtAll = @(Get-AssocRows @((New-Sample62 'start' $refusedEn 1 (New-ApiReading62 1 0 149 'on' 'on' 'open')), (New-Sample62 'end' $refusedEn 1 (New-ApiReading62 1 0 149 'on' 'on' 'open'))))
Assert-Equal '#62 assoc: neither reader listing anything is the one no-interface row, its first line the none token' ("{0}/{1}" -f $noneAtAll.Count, ((($noneAtAll[0].Details -split "`r`n|`n")[0]) -match '[:：]\s*none\s*$')) '1/True'
