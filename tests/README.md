...
| `gui` | `gui_check.ps1`, both languages × both entries, on staged copies (the IT copy configured with 30 pings / 125 s) | the window is found; the IT title carries the IT marker and the user title does not; the IT run does not start by itself and Start is clicked with the panel untouched; the JSON report's `EntryPoint` matches the entry and `PingCount` / `SampleSeconds` match the folder's configuration (30 / 125 for IT: not clamped to the spinner defaults); the window is closed through its Close button; exit 0 | EntryPoint User 4 / 8, EntryPoint IT 30 / 125, 28 results |
| `acceptance` | `NetworkHealthCheck.ps1 -ConsoleOnly` on staged copies: en-US user, en-US with the IT switches (`-PingCount 6 -SampleSeconds 6 -PingTarget 8.8.8.8 -TcpTarget 1.1.1.1:53 -TracerouteHops 4 -ExpandDetails`), zh-TW user | exit 0 and a JSON report whose run options match the launch (entry point, ping count, sample seconds, hops, extra targets present); `-RequireHealthy` adds Overall Healthy | Overall Healthy; 28 / 30 / 28 results |
| `package` (`-Package`) | `build_asset.py`, then `validate_release.py` run from inside the extracted ZIP, then the extracted IT entry through the real window in both languages | the asset builds from tracked files only with CRLF intact; the packaged validator passes inside the package; the IT window opens from the extracted files | 66 / 66; SHA256 printed for the release notes |
...
| `gui_check.ps1` | Drives the real window through UI Automation the way a person would: launches the script, finds its window, for the IT entry lists the panel's fields and clicks Start with the panel untouched, waits for the JSON report, prints the run options it recorded, and closes the window with its Close button. The folder must be named `en-US` or `zh-TW` (the button names are per language). The spinner values cannot be read back through UI Automation, so the JSON report is the evidence. | `-PackageDir <copy of healthcheck\en-US or zh-TW> -Entry User|IT [-TimeoutSeconds 300]` |
...
- `unit_tests.ps1`, `report_stage_tests.ps1` and `gui_check.ps1` carry a UTF-8 byte-order mark on purpose: Windows PowerShell 5.1 reads a script without one in the system code page, and these three contain non-ASCII text (zh-TW assertions and button names).
...
- `parse_check.ps1`, `unit_tests.ps1`, `report_stage_tests.ps1`, `gui_repro.ps1` and `gui_check.ps1` are the scripts that produced the 1.2.1 record, committed as they ran (`report_stage_tests.ps1` was `report_stage_tests_v121.ps1`); `selftest_guards.py` and `build_asset.py` differ from their scratchpad versions only in resolving paths from the repository root and reading the v1.2.0 anchors from git.
