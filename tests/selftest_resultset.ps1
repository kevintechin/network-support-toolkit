...
# The interface GUIDs the facts read off netsh (round 11): the first GUID-shaped value of each blank-line-separated block,
# so a network or profile named like a UUID is not an interface, and two blocks are two interfaces.
...
Assert-Case 'netsh: a network and a profile named like a UUID are not interfaces, and two blocks are two' @($(if (($readIds -join ',') -eq '0b3f7c2e-1111-4a2b-9c3d-000000000002,e6b08c8a-3feb-4c3e-88c3-dee94dd2f0eb') { @() } else { @('read ' + ($readIds -join ',')) })) $true ''
