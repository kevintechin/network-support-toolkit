...
$wifiWiredLook = New-Row $assocTemplate 'wifi' 'Wi-Fi radio' 'INFO' 'IT'; $wifiWiredLook.Details = 'Wireless interfaces reported by netsh: 0'
...
$r = New-Fixture; $r.Results = @($r.Results | Where-Object { $_.Tag -notin @('wifi', 'wifi-association') }) + @($wifiWiredLook, $assocReaderFailed); Assert-Case 'netsh refused and the WLAN API reader failed: the aggregate rows without a GUID are the right shape' @(Test-ResultSet $r $cfg @{} $refusedFacts) $true ''
