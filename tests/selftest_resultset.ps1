...
# backlog #62: netsh refused by the location setting - the radio row and the association row are written from the WLAN service,
# one per interface the facts name by its adapter's GUID, each naming that GUID; the wired-looking rows a refused netsh once
# produced are refused by the GUID they do not name.
$refusedFacts = With $facts @{ WlanRefused = $true; WifiInterfaces = 1; WlanInterfaceIds = @('e6b08c8a-3feb-4c3e-88c3-dee94dd2f0eb') }
$wifiRefused = New-Row $assocTemplate 'wifi' 'Wi-Fi radio' 'INFO' 'IT'; $wifiRefused.Details = 'Interface (WLAN service): Fixture AX211' + [Environment]::NewLine + 'Interface GUID: e6b08c8a-3feb-4c3e-88c3-dee94dd2f0eb' + [Environment]::NewLine + 'Connection state: connected (WLAN service); netsh exit code 1; connection query: error 5: Access is denied'
$assocRefused = New-Row $assocTemplate 'wifi-association' 'Wi-Fi association' 'INFO' 'IT'; $assocRefused.Details = 'Sample 1 (before the first measurement, 10:00:00): not sampled - netsh wlan show interfaces exited with code 1 and printed no interface; WLAN service: connected' + [Environment]::NewLine + 'Interface GUID: e6b08c8a-3feb-4c3e-88c3-dee94dd2f0eb; samples=start,middle,end'
$wifiWiredLook = New-Row $assocTemplate 'wifi' 'Wi-Fi radio' 'INFO' 'IT'; $wifiWiredLook.Details = 'Wireless interfaces reported by netsh: 0' + [Environment]::NewLine + 'WLAN service: 1 wireless interface(s) listed; wlanapi=ok'
$wifiReaderFailedRow = New-Row $assocTemplate 'wifi' 'Wi-Fi radio' 'INFO' 'IT'; $wifiReaderFailedRow.Details = 'Wireless interfaces reported by netsh: 0' + [Environment]::NewLine + 'WLAN service: not read - addtype fixture; wlanapi=addtype'
$r = New-Fixture; $r.Results = @($r.Results | Where-Object { $_.Tag -notin @('wifi', 'wifi-association') }) + @($wifiRefused, $assocRefused); Assert-Case 'netsh refused: the radio row and the association row written from the WLAN service' @(Test-ResultSet $r $cfg @{} $refusedFacts) $true ''
$r = New-Fixture; $r.Results = @($r.Results | Where-Object { $_.Tag -notin @('wifi', 'wifi-association') }) + @($wifiRefused, $assocWired); Assert-Case 'netsh refused: the no-interface association row on a machine whose adapter the facts name' @(Test-ResultSet $r $cfg @{} $refusedFacts) $false 'wifi-association: interface e6b08c8a-3feb-4c3e-88c3-dee94dd2f0eb has 0 row(s)'
$r = New-Fixture; $r.Results = @($r.Results | Where-Object { $_.Tag -notin @('wifi', 'wifi-association') }) + @($wifiWiredLook, $assocRefused); Assert-Case 'netsh refused: a radio row naming no interface GUID is the old wired-looking row, refused' @(Test-ResultSet $r $cfg @{} $refusedFacts) $false 'wifi: a row without an interface GUID'
$r = New-Fixture; $r.Results = @($r.Results | Where-Object { $_.Tag -ne 'wifi' }) + @($wifiWiredLook); Assert-Case 'netsh answering (not refused): the same GUID-less radio row is not held to the rule' @(Test-ResultSet $r $cfg @{} $facts) $true ''
# PR #55, round 4: netsh refused and the tool's own WLAN API reader failed too - the aggregate rows carry the reader's reason
# as a token, and the facts' substituted GUIDs are not demanded; with the reader answering (wlanapi=ok) they still are.
