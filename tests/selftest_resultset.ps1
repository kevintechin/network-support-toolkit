...
$r = New-Fixture; $r.Results = @($r.Results | Where-Object { $_.Tag -ne 'wifi-association' }) + @($assocForeign); Assert-Case 'an association row with a GUID line but no samples token is a tool regression, refused as such' @(Test-ResultSet $r $cfg @{} $facts) $false 'wifi-association: a per-interface row carries no samples token'
