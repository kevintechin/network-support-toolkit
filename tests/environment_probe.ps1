...
Add-List 'IP configuration (Win32_NetworkAdapterConfiguration, IP-enabled: description | addresses | gateways | DNS | DHCP | DHCP server)' { Get-CimOrWmi 'Win32_NetworkAdapterConfiguration' | Where-Object { $_.IPEnabled } | ForEach-Object { '{0} | {1} | {2} | {3} | DHCP={4} | DHCPServer={5}' -f $_.Description, (@($_.IPAddress) -join ','), (@($_.DefaultIPGateway) -join ','), (@($_.DNSServerSearchOrder) -join ','), $_.DHCPEnabled, $_.DHCPServer } }
...
Add-Fact 'Wireless interfaces listed, connected or not (netsh wlan show interfaces, interface GUID lines; the set the Wi-Fi retry row writes one row per)' { @(Invoke-Exe 'netsh.exe' @('wlan', 'show', 'interfaces') | Where-Object { $_ -match '[0-9a-fA-F]{8}-([0-9a-fA-F]{4}-){3}[0-9a-fA-F]{12}' }).Count }
