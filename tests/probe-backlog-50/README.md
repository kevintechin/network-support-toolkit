...
- **As a double-click.** The `.cmd` is handed to the shell with its folder as the working directory, which is what
  ...
  a window, the whole primary screen where another process hosts the console — and only then closed. The picture is
...
bytes on standard error, six `is not recognized` pairs. Five come from Chinese `rem` comments in the first quarter
of the file and have scrolled off by the time the run ends, which is why the walk recorded the sixth and only the
sixth. A copy with its last line in ASCII loses the sixth message and keeps the five.
