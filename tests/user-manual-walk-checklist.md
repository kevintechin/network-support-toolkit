...
**Then the capture harness, which owns 51 of these rows.** `walk_capture.ps1` runs the tool the way the rows below do and keeps what it saw - the window at whatever size the desktop resolved it to, the report's three files, the launcher's error report, the folder listings, the Mark of the Web and the dialog it raises - and writes one bundle with a `manifest.md` binding each answer to its row. Fill the *Observed* column of those rows from the manifest; a row it could not reach says `not produced` and names the precondition that was missing, so that no row is silently absent. Several rows are answered `not produced` by design rather than by circumstance: W5b needs a second, unblocked extraction the script does not make, W34 needs the three numeric spinners moved and this panel exposes no way to move them, and W49 and W51 are conditions the sheet forbids manufacturing. Their artefacts are still in the bundle where the script produced any. It refuses to run when the session cannot be captured, and when the bundle would land in a synced folder.
...
| | Rows |
|---|---|
| Captured by the harness, when the machine allows it | W1, W5, W7, W9, W9b, W10, W12, W14, W15, W16, W17, W18, W20, W21, W22, W24, W25, W27, W28, W29, W31, W32, W38, W39, W51, W53, W56, W57, W58, W60 |
| Answered `not produced`, with the precondition named | W2, W3, W4, W5b, W13, W33, W34, W36, W37, W40, W42, W43, W44, W45, W46, W47, W48, W49, W50, W54, W59 |
...
| W4 | In the same ZIP window, open `en-US\` and double-click `Start-NetworkCheck.cmd` | The black window says `The program file NetworkHealthCheck.ps1 is missing`, and names `LauncherError_<stamp>.txt` | | the window's text, the file |
...
| # | Do this | Expected — the manual's words | Observed | Evidence |
|---|---|---|---|---|
| W28 | Rename `en-US\NetworkHealthCheck.ps1` to `NetworkHealthCheck.ps1.bak`, double-click `en-US\Start-NetworkCheck.cmd`, then rename it back | The window says **ERROR: The program file NetworkHealthCheck.ps1 is missing**; `LauncherError_<stamp>.txt` appears beside the launcher with the time and this computer's short-date pattern beside it, the computer, the user, the reason, and a **Suggested action** line that fits the reason (extract the complete ZIP) | | the file |
| W29 | Open the `LauncherError_<stamp>.txt` from W28 and read it against the manual's description of it | The file holds what the manual says it holds: the time with the short-date pattern beside it, the computer, the user, the folder, the program's path, the PowerShell it would have used, a `PowerShell messages:` line saying PowerShell was not started, the reason, and a **Suggested action** line that fits the reason. The file does **not** say which file to send — that instruction is the manual's, in the same section-6 row — so check the two separately: the action from the file, the send from the manual | | the file |
...
| W44 | **Another exit code** — the campaign's `M8`: a Group Policy execution policy of *Allow only signed scripts*, then revert | The window says the program ended with another exit code and that PowerShell or company security policy may have blocked it; PowerShell's own reason is kept in `PowerShellMessages_<stamp>.txt` beside the launcher and printed back under the error, and `LauncherError_<stamp>.txt` says to send both files | | the window's text, the two files |
| W45 | **The `%TEMP%` copy of the launcher's file** — in this order: rename `NetworkHealthCheck.ps1` away, then deny write on the `en-US` folder for this account, then double-click the launcher; restore the permission and the file afterwards. The rename happens while the folder is still writable. (Until 1.2.14 the file W28 left had to be deleted first, because the launcher asks `if exist` after its write and a stale report under the fixed name satisfied that check; the name carries a stamp now, so a stale file cannot) | The launcher writes `NetworkHealthCheck_LauncherError_<stamp>.txt` in the Windows temporary folder instead, with the same lines, and **the window prints the path** | | the file |
...
| W52 | **The black window flashes and closes** — PowerShell stopped before the tool could say anything. The campaign's `M9` (AppLocker enforced) is the scenario that would produce it, and it has never taken effect on any machine this project has measured ([backlog #31](../docs/backlog.md)), so record *not produced* unless it happens | Nothing is written — no report, no `LauncherError_<stamp>.txt`, no environment file — and the manual's advice is to tell IT what was seen, with a photo of the window if one can be caught. This is the case where the package leaves no evidence at all ([backlog #37](../docs/backlog.md)) | | |
