...
try {
    ...
    if ($selected -contains 'parse') {
        # And no packaged file whose bytes on this disk are not the bytes a checkout produces. SHA256SUMS.txt is
        # regenerated by hashing the working tree, and `build_asset.py` packs the working tree too, so a file whose
        # line endings here differ from what `.gitattributes` gives a fresh clone ships a digest only the machine
        # that built it can verify. Measured on PR #69 round 3: an edit through Python's `write_text` turned
        # `healthcheck/tools/validate_release.py` from LF into CRLF on this disk, git normalised it back to LF on
        ...
        Invoke-Case 'parse' 'every packaged file is on disk as a checkout would write it' {
            ...
            foreach ($line in $eol) {
                ...
                if ($line -notmatch '^i/(\S+)\s+w/(\S+)\s+attr/(.*?)\s*\t(.+)$') { $bad += ('unreadable: ' + $line); continue }
                $work = $Matches[2]; $attr = $Matches[3]; $path = $Matches[4]
                if ($attr -match 'eol=(lf|crlf)') {
                    $want = $Matches[1]
                    if ($work -ne $want -and $work -ne 'none') { $bad += ('{0}: {1} declares {2} and this disk has {3}' -f $path, '.gitattributes', $want, $work) }
                }
            }
            @{ Passed = ($bad.Count -eq 0); Detail = $(if ($bad.Count) { $bad -join '; ' } else { ('{0} tracked file(s), each on disk as a checkout would write it' -f $eol.Count) }) }
        }
