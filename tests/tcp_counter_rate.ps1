...
<#
    ...
    and TCPv6 in turn and gives each read up to two attempts. Per condition it reports, over 2 x Iterations reads:
      ...
      read time               - how long one complete read of both classes took, in a snapshot with no failed attempt:
                                median, 95th percentile and maximum per condition, and each class at the median (backlog
                                #65: what a read inside the sample window costs, measured with the reader the window uses;
                                PR #56 round 1 made it the whole pass and the percentile the release evidence quotes).
...
function Measure-Condition {
    ...
    for ($i = 1; $i -le $Count; $i++) {
        ...
        # The complete pass - both classes, in order, as one read inside the window makes it - from the start to the
        # snapshot's own closing stamp (PR #56, round 1: a per-class figure understates what one interior read costs).
        $passMs = $null
        if ($cleanSnapshot -and $snapshot.Counters.Count -eq 2) { $passMs = [math]::Round(($snapshot.Timestamp - $started).TotalMilliseconds, 1) }
...
function Write-ConditionSummary {
    ...
    # backlog #65: the cost of a read that behaved - the complete pass of both classes, once per snapshot, from the
    # snapshots of this condition that no failed attempt delayed; and each class at the median beside it.
    $passes = @($Reads | Where-Object { $_.Protocol -eq 'TCPv4' -and $null -ne $_.PassMs } | ForEach-Object { [double]$_.PassMs } | Sort-Object)
    if ($passes.Count -gt 0) {
        $at = { param([double[]]$values, $q) $values[[math]::Min($values.Count - 1, [math]::Max(0, [int][math]::Ceiling($q * $values.Count) - 1))] }
        $v4 = @($Reads | Where-Object { $_.Protocol -eq 'TCPv4' -and $null -ne $_.ReadMs } | ForEach-Object { [double]$_.ReadMs } | Sort-Object)
        $v6 = @($Reads | Where-Object { $_.Protocol -eq 'TCPv6' -and $null -ne $_.ReadMs } | ForEach-Object { [double]$_.ReadMs } | Sort-Object)
        Write-Output ("       one complete read of both classes, in a snapshot with no failed attempt: {0} passes, {1} ms at the median, {2} ms at the 95th percentile, {3} ms at most (TCPv4 {4} ms and TCPv6 {5} ms at the median)" -f $passes.Count, (& $at $passes 0.5), (& $at $passes 0.95), $passes[-1], $(if ($v4.Count) { & $at $v4 0.5 } else { '-' }), $(if ($v6.Count) { & $at $v6 0.5 } else { '-' }))
    }
