...
- **Read state:** client sees the SSID? AP client table — associated vs **authenticated**; RSSI ≥ **−65 dBm**, SNR ≥ **25 dB** as the AP sees them (client side: `netsh wlan show interfaces`; the report's `wifi-retry` row — the adapter's own 802.11 retry rate over the run, the air's signature, since retries are absorbed as delay and show in neither the TCP nor the ping figures); RADIUS log — **Access-Reject** is fast and appears in the server log; a **timeout** is slow (AP retries) and the server log stays silent; channel utilization and client count.
