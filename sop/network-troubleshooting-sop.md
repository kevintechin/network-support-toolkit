...
- **Read state:** client sees the SSID? AP client table — associated vs **authenticated**; RSSI ≥ **−65 dBm**, SNR ≥ **25 dB** as the AP sees them (client side: `netsh wlan show interfaces` — on Windows 11 24H2 and later it prints no interface unless desktop apps may use the location, and the report's `wifi` row then says so; the report's `wifi-retry` row — the adapter's own 802.11 retry rate over the run, the air's signature, since retries are absorbed as delay and show in neither the TCP nor the ping figures; its `wifi-association` row — the access point at three moments of the run, a roam under the same SSID named, a roam out and back between two samples invisible and said so; and the `gateway-neighbor` row's hint on whether the access point is the gateway itself); RADIUS log — **Access-Reject** is fast and appears in the server log; a **timeout** is slow (AP retries) and the server log stays silent; channel utilization and client count.
