...
| `traceroute` | IT | The first hops, each with its address, status and time; silent hops as `*`, and whether the destination was reached | **It samples the first hops only — three by default** (`TracerouteHops`), which is what the row's name says. The last answering hop names the boundary **only if the row says the destination was reached**; otherwise it is merely the last hop the tool looked at. Run the full `tracert` from the Manual-check line before assigning the fault to anyone's domain |
