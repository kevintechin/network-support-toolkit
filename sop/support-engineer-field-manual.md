...
**If you asked for a target, read its row whatever the verdict says.** The verdict speaks for **required** targets only. A target that is not marked required — which includes every one you add through the IT panel or the switches — reports `INFO` when it fails outright and `WARN` when it merely degrades, and **never moves the verdict**. So a run in which the service you were asked about was completely unreachable can still come back **Overall Healthy**, with that failure sitting in an `INFO` row the four moves above would skip. Measured on a real run: three added targets, all unreachable (100 % loss, no TCP connection, no HTTPS response), all `INFO`, verdict `Attention Required` for an unrelated 50 % loss on a configured target.
...
**A healthy report does not clear the network.** It clears *this endpoint's view, at that moment*. Say that in the ticket rather than "the tool says it's fine". And because the fingerprint is computed from the verdict-bearing rows alone, it says nothing about the targets you added: **`healthy` with a dead optional target is a normal outcome**, not a contradiction (§3).
...
| Row | Scope | Testifies to | Limit |
|---|---|---|---|
| `ping-gateway`, `ping-target` | Main | The ladder's rungs, with loss and latency | ICMP may be filtered; that is why the next two rows exist. **The status depends on whether the target is *required***: no reply at all → FAIL when required, `INFO` when not; above the **critical** loss or latency threshold → FAIL when required, WARN when not; above the **warning** threshold → WARN either way. Everything added at run time is optional |
