...
**Exit codes**, if you are scripting around it, hold for **console runs only** (`-ConsoleOnly`, which is what the console launcher starts): there `0` means the run finished and wrote its reports — *whatever the verdict was* — and anything else means **no normal report** — which is not the same as no evidence: exit 3 writes `NetworkHealthCheck_ENVIRONMENT_*.txt` first, and a run that could not write a single format writes `NetworkHealthCheck_FATAL_*.txt` before returning 1. Collect the path the process printed instead of discarding the run (§6). **A GUI run always exits 0.** The entry point sets it after the window closes, so an IT window closed before Start was ever clicked, and a run whose report writing failed, both exit 0 as well. Never read a GUI exit code as evidence; look for the report.
