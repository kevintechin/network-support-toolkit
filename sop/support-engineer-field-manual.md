...
| `gateway-unreachable` · Gateway does not answer | A `ping-gateway` row failed **on its loss** — replies that did not come back; the row's `Rule` field says `loss` — and none passed, decided **before** the TCP, HTTP and group rows are considered, so a gateway that forwards traffic but drops ICMP to its own address lands here with the rest of the network working. A gateway that answered every probe slowly (`Rule` = `latency`) does **not** land here: it is `quality`, or `mixed` beside another failure | **IP, gateway unreachable** — local L1/L2, *once the ping is corroborated*. This is SOP Rule 3 in its purest form: a failed ping is a suspect, not a conviction | **2** (wireless) or **3** (wired) — but read the `ping-near-end` row, if the site configured one, and the `tcp`, `http`, `connectivity-group` and `gateway-neighbor` rows first | A `ping-near-end` row that **passed** through the same adapter as the failed gateway (both rows' `Path`) clears that adapter's local path for that run and leaves the gateway itself — the summary's second line already says so; one that **lost its replies** points at the local path before the gateway — the wireless or wired station is the first move, with the host itself the other possibility. Without one: if those passed **and at least one of them was off-subnet by a route through the gateway in the row you are reading** — the targets are configured independently, so an on-subnet host, a VPN or a proxy can succeed without ever touching it, and on a multihomed machine `AUTO_GATEWAY` produces **one row per gateway** while the fingerprint asks only that none passed, so a witness routed through gateway A says nothing about failed gateway B — the path works and that gateway is merely silent to ICMP: stay at station 1 and look at the config rather than walking to the switch. Work through any other failed gateway row on its own terms. If they failed too: port error-counter delta, two samples; MAC learned on the expected port and VLAN; for Wi-Fi, the AP client table — the report's own RSSI is the client's view, weaker evidence |
| `gateway-up-internet-dead` · Gateway answers, internet does not | A gateway ping passed, a `connectivity-group` failed and none passed. **The rule does not read the group's name** — a site that replaced the Internet group with an internal one (the package's IT deployment manual tells IT how) lands here for a failed internal service — and it pairs **any** passing gateway row with **any** failed group without checking that they share a route: on a multihomed machine gateway A can answer while the failed group's traffic leaves through the unavailable gateway B | **Gateway OK, internet dead** — *only when the failed group is the Internet one, and only when the gateway that answered is the one its members route through.* Read which group failed, what its members target, and which gateway that traffic uses, before walking outward | **5 → 6 → 8** for an Internet group whose route uses the gateway that answered; where a different gateway carries it, the fault is local routing or that second link — stay at **1**, then **3** | Default route present at the gateway; NAT translation and policy hit counters; WAN status. The report's traceroute row traces the **first configured ping target, not the group member that failed**, and only three hops by default — so unless those two are the same destination it says nothing about this failure. Ask for an uncapped `tracert -d <the address that failed>`; the row's own Manual-check line carries both the wrong target and the three-hop cap |
...
| `quality` · Connected, but quality is poor | A quality row (`ping-target`, `ping-gateway`, `ping-near-end`, `tcp-retransmissions`, `adapter-errors`) warned or failed and **no other row warned or failed** — a gateway that answered every probe slowly included: its row's `Rule` is `latency`, and it lands here rather than under `gateway-unreachable`. Only WARN and FAIL exclude it: an unrelated **ERROR** row does not, and this rule is evaluated before the ERROR one — so a `Test Incomplete` verdict can arrive carrying a `quality` fingerprint | **Intermittent** — front door Q4 | **2 → 3** | Counter deltas at the port; RF numbers from the AP; a second run **while the problem is happening**. If the verdict is `Test Incomplete`, deal with the ERROR rows as well (§6) — the fingerprint did not consider them |
...
**The report now tells you which server answered.** Each adapter row's details carry a `DHCP server:` line — the address that answered the lease, read from `Win32_NetworkAdapterConfiguration.DHCPServer` on both data-source paths — which is SOP station 1's rogue-DHCP question. A static address names no server; where the class could not be read the line says *unavailable*, and only then do you still need `ipconfig /all`'s `DHCP Server` line from the user.
...
| `gateway-neighbor` | IT | The gateway's MAC **as the neighbour cache holds it**, with the entry's state | A cache read, not a probe: `Get-NetNeighbor` / `arp -a` report an entry that may be stale, so presence alone proves nothing about the link *now*. A `Reachable` entry is Rule 3's second witness for a failed gateway ping; a stale one is history — corroborate it with the TCP or HTTP rows before convicting or clearing the link. Since 1.2.13 the row also compares the gateway's MAC with the BSSID of the wireless interface that supplied the gateway and says what the two addresses suggest — one device, probably one, the same maker, or two — as a hint, never a topology claim; identical addresses mean the network has no wire between the air and the gateway for the three-way comparison to stand on. No line where the gateway's adapter is wired or `WifiRf` is off |
| `proxy`, `drivers`, `wifi` | IT | Proxy configuration, adapter driver versions, the client's own view of the radio | `wifi` is the client side: SSID, band, channel, RSSI, rate. The AP's client table is the stronger witness (SOP station 2). Since 1.2.13 the connection state is the WLAN service's, not parsed from netsh, and a connected machine whose `netsh wlan show interfaces` prints no interface because desktop apps may not use the location (Windows 11 24H2 and later) gets a row saying the interface is connected and which fields could not be read — not the wired-looking row it got before (backlog #62). **`drivers` is empty on the CIM fallback path** — the fallback leaves the version, date and provider blank by construction, and the row still appears showing `(none)`; check the adapter's `Data source:` field, and read `Get-NetAdapter \| Format-List Name, DriverVersion, DriverDate` instead of that row |
| `wifi-association` | IT | Which access point (BSSID) each wireless interface was associated with at three moments of the run — before the first measurement, with the IT diagnostics, after the last — and whether it changed, since 1.2.13 | A roam under the same SSID during the run explains a good signal with bad numbers, and the row lists the samples with their times; but the access point is **sampled, not watched**: a roam out and back between two samples is invisible, and the row says so every time. No BSSID at a sample is *not reported*, never disconnected, and since 1.2.13 the WLAN service's state stands beside it; a sample at which `netsh` printed no interface — the location setting — reads *not sampled*, and an interface refused at every sample gets its own row, not the wired machine's (closed item #62). Written whether or not there is a radio — a wired machine gets one row saying no interface was listed — and absent only when `WifiRf` is off, in the config or with `-NoWifi` |
...
| `tcp-retransmissions` | Main | Approximate retransmission ratio over the sample, TCPv4 and TCPv6 | It measures **whatever the machine was doing at the time**. A machine that was busy over a weak Wi-Fi during the sample will look bad; this was measured in the tool's own validation. A single high reading is a suspect, not a conviction. Since 1.2.12 the `tcp` rows carry the figure this one cannot — the tool's own handshakes to one target, with the SYNs each socket had to send again — and this row says so when a run took that sample. Since 1.2.14 the row also places its retransmissions inside the window — the counters are read again at least every `RetransmissionIntervalSeconds` (2 s as shipped) — and names the interval that held the most of them with its share of the count and of the time: evidence about distribution, never a verdict, and coarse or absent where a read inside the window failed and stopped the reads (backlog #65) |
| `wifi-retry` | Main | The wireless adapter's 802.11 retry rate over the run — frames sent again because no acknowledgement came back, over frames tried (transmitted plus abandoned) — one weightless row per wireless interface, since 1.2.12 | It is **the air between this client and its access point**, for whatever the machine was doing, and it is neither the TCP retransmission rate nor the ping loss figure: a retried frame usually arrives, later. No threshold has a basis, so it decides nothing — read it beside the RSSI and beside the same machine's figure on a good day; a high rate with a clean verdict is a radio working hard. On a wired machine an INFO row says there is no wireless interface; where the reader could not be compiled or the WLAN service did not answer, an *Unable to Check* row says which. The driver's word: the reference machine's Intel driver fills the counters, others have not been measured |

**Probes — differential tests**

| Row | Scope | Testifies to | Limit |
|---|---|---|---|
| `ping-gateway`, `ping-target` | Main | The ladder's rungs, with loss and latency | ICMP may be filtered; that is why the next two rows exist. **The status depends on whether the target is *required*, and the tests are an ordered chain — the first condition that matches decides.** No reply at all → FAIL when required, `INFO` when not. Then loss ≥ critical → FAIL / WARN. Then loss ≥ warning → WARN. Then latency ≥ critical → FAIL / WARN. Then latency ≥ warning → WARN. Because loss is tested before latency, **a required target with warning-level loss and critical latency is WARN, not FAIL** — the chain stops at the first match, it does not pick the worst. Everything added at run time is optional. Since 1.2.12 the row's `Rule` field says which of the two decided it — `loss` or `latency` — and the gateway row's details carry its rung line — the local path, answered by the gateway's control plane, and nothing beyond it — claimed only where the route table selected an on-link source on the supplying adapter's subnet before and after the probes; `Path` on a near-end or gateway row is the adapter it claimed its rung on, empty where it could not - and empty on every other ping row. Since 1.2.13 every ping row with two or more replies also carries a **Spread** line — the standard deviation of the round-trip times that arrived, over how many replies — a description with no threshold: whole-millisecond times make a spread of about 1 ms at millisecond latencies the clock's resolution, and below nine replies the figure describes those replies rather than the link, which the line says. Variability with little loss is the air's signature one layer above the `wifi-retry` row; read the two together |
| `ping-near-end` | Main | The near-end rung: a host on the client's own subnet that is not the gateway, so its loss and latency are the local path — adapter, cable or Wi-Fi, switch or access point — with neither the gateway's control plane nor the WAN in the figure | Only where the site configured `Tests.NearEndTarget`; a target that is the gateway, this computer's own address, a subnet's network or broadcast address, a name, or an address outside the primary adapters' subnets is not probed and the row says why (a weightless row, plus the *did not run* row where it was required; a required one with no address is those two rows too). And a measured row claims the rung — and carries this tag — only where the route table selected an on-link source on the target's subnet before and after the probes; where a VPN, a second connection or a more specific route was selected, the row says so and is tagged `ping-target`, so the summary does not read it. **Read the ladder in order**: a rung that passes clears what it crossed at that moment, the first rung that fails puts the problem beyond the last rung that passed and no closer, and **two rungs' loss figures never subtract into a segment figure** — they are separate traffic at separate moments. The `gateway-unreachable` summary's second line reads this row only where its `Path` matches every failed gateway row's: passed → the gateway itself; lost its replies → the local path before it, or the host itself |
| `tcp`, `http` | Main | A real connection to a real port, and an HTTP response | Rule 3's corroboration — but only between probes that name the **same destination**. The ping, TCP and HTTP targets are configured separately and the IT panel can add more, so read the targets in the rows first: "ping fails, TCP succeeds" is a filtering finding when both went to the same host, and nothing at all when they did not. Since 1.2.12 a `tcp` row that connected lists `PingCount` handshake times (four as shipped) and, per connection, the SYNs the socket had to send again — its own count, read through `SIO_TCP_INFO` on Windows 10 1703 / Server 2016 or later; an older build reads the times against the initial retransmission timeout instead and calls that a proxy — which is the one retransmission figure in the report attributable to a single destination: this machine's own SYNs, to that target, at that moment. It moves no verdict; a repeated connection that failed is named by its position and ends the repeats |
...
|---|---|---|
| `LauncherError_<stamp>.txt` — one per attempt since 1.2.14 | The launcher stopped before or after the program. Since 1.2.3 it carries **`Suggested action:`** matched to the reason, and the same line is on screen. Since 1.2.14 its `Date/time:` line states the machine's short-date pattern beside the shell's own date, its `PowerShell messages:` line says whether PowerShell printed anything and where it is kept, and the digits in the name are the date and time as that machine prints them | Read the suggested action first; it is reason-specific, not boilerplate. When the report names a `PowerShellMessages_<stamp>.txt`, ask for it with the same stamp: it holds PowerShell's own refusal, the sentence the person used to be asked to retype. The file's modified time is the stamp to trust, whatever format the `Date/time:` line is in. The files are written beside the script, or — when that folder cannot be written, which is exactly what a read-only share or a protected folder produces — in `%TEMP%` as `NetworkHealthCheck_LauncherError_<stamp>.txt` and `NetworkHealthCheck_PowerShellMessages_<stamp>.txt`, the same lines. The launcher checks the first write and **not** the second, so it prints that `%TEMP%` path whether or not the file arrived: ask for the file, not just the path, and if it is not there ask for what the window said |
...
- **The quality rows measure the machine's own traffic.** A big download, a backup, or a busy remote session during the sample will move the retransmission ratio. Ask what the machine was doing. The connection times on the `tcp` rows are the exception: they are the tool's own handshakes and nobody else's.
- **A machine with no wireless adapter still produces a `wifi` row** — an INFO row saying no wireless interface is connected, listing each interface the WLAN service or `netsh` reported with its state and radio switch, and the counts from both readers beneath (since 1.2.13; until then the row guessed at wired, radio off or no adapter). A connected machine whose `netsh` prints no interface because desktop apps may not use the location is not that row: it gets a row per connected interface naming the fields that could not be read and the setting (backlog #62). Even a machine without `netsh.exe` gets a row saying that. In a run that reached the IT diagnostics at all, the *absence* of any `wifi` row means one thing: the optional `WifiRf` check is switched off in the config. A report that stopped before them — an unsupported Windows version, or PowerShell below 5, each of which writes a normal report and returns early — carries no `wifi` row either, so read the environment rows before concluding anything. Never read a disabled or unreached diagnostic as evidence about the hardware. The `wifi-retry` row follows the same rule one step further: it is written whether or not there is a wireless adapter — no adapter is an INFO row, a reader that could not be compiled is an *Unable to Check* row — so its *absence* means `WifiRetryCounters` is off in the config, or the run stopped before the counters. The `wifi-association` row rides the `wifi` row's own switch: written whether or not there is a radio — a wired machine gets one row saying no interface was listed — and absent only when `WifiRf` is off or the run stopped before the samples.
- **A machine that has only Wi-Fi cannot make the three-way comparison** (SOP, segment probe), and since 1.2.13 its report reads the signs for you, in this order: the `gateway-neighbor` row's hint on whether the access point is the gateway itself — identical addresses mean there is no wire on that network to compare against, and the comparison is over before it starts; the `wifi-association` row — did the computer change access point during the run (a roam out and back between two samples stays invisible, and the row says so); the `wifi-retry` row — retries absorbed as delay are the air's signature; the Spread lines of the ping rows beside their loss — variability with little loss is the same signature one layer up, and below nine replies it describes those replies only; the `ping-near-end` rung if the site configured one; and the RSSI on the `wifi` row. The fastest checks are still human: a second station on the same access point, a change of position or band, and a cable once if any port exists.
...
- The package's own user manual (`NetworkHealthCheck_User_Manual_<lang>.md` / `.html`, what the person at the machine is told), IT deployment manual (`NetworkHealthCheck_IT_Deployment_Manual_<lang>.md` / `.html`, what IT configures, deploys and verifies, and what security policy does to the tool) and technical guide — what the tool does and how it decides
