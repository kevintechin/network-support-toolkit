...
| Row | Scope | Testifies to | Limit |
|---|---|---|---|
| `ping-gateway`, `ping-target` | Main | The ladder's rungs, with loss and latency | ICMP may be filtered; that is why the next two rows exist. **The status depends on whether the target is *required***: a required target with no reply is FAIL, an optional one is `INFO`; degraded, they are WARN either way. Everything added at run time is optional |
